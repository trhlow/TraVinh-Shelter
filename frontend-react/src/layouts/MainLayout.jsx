...
export function Header({ session, onLogout }) {
  ...
  return (
    <header className="bg-surface-container-lowest/95 border-b border-outline-variant docked full-width top-0 sticky z-50 w-full backdrop-blur transition-all duration-300">
      <div className="flex justify-between items-center px-margin-mobile md:px-margin-desktop py-4 max-w-container-max mx-auto">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a className="text-on-surface-variant hover:text-trust-navy transition-colors px-2 py-1 rounded font-label-bold text-label-bold" href="#/projects">
            Dự án
          </a>
          <a className="text-on-surface-variant hover:text-trust-navy transition-colors px-2 py-1 rounded font-label-bold text-label-bold" href="#/brokers">
            Môi giới
          </a>
        </nav>
        ...
        <div className="flex items-center gap-3">
          {!session && (
            <a className="hidden min-h-11 items-center justify-center rounded border border-outline-variant bg-surface-container-lowest px-4 py-2 font-label-bold text-label-bold text-primary transition-colors hover:bg-surface-container-low md:flex" href="#/login">
              Đăng nhập
            </a>
          )}
          {isAdmin && (
            <a className="hidden min-h-11 items-center justify-center rounded border border-outline-variant bg-surface-container-lowest px-4 py-2 font-label-bold text-label-bold text-primary transition-colors hover:bg-surface-container-low md:flex" href="#/admin/overview">
              Admin
            </a>
          )}
          {isBroker && (
            <a className="hidden min-h-11 items-center justify-center rounded bg-action-orange px-4 py-2 font-label-bold text-label-bold text-on-primary shadow-sm transition-colors hover:bg-orange-600 md:flex" href="#/broker/properties">
              Đăng tin
            </a>
          )}
          {session && (
            <button className="ui-secondary-action text-on-surface-variant" onClick={onLogout}>
              Đăng xuất
            </button>
          )}
        </div>
...
function NavDropdown({ label, sections }) {
  return (
    <div className="relative group">
      <button
        className="flex items-center gap-1 text-on-surface-variant hover:text-trust-navy group-hover:text-trust-navy hover:bg-surface-container-low transition-colors px-2 py-1 rounded font-label-bold text-label-bold"
        type="button"
        aria-haspopup="true"
      >
        {label}
...
export function Footer() {
  return (
    <footer className="w-full border-t border-outline-variant bg-surface-container-lowest py-stack-lg mt-auto">
      <div className="flex flex-col md:flex-row justify-between items-center px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto gap-4 md:gap-0">
        <div className="font-headline-sm text-headline-sm text-trust-navy font-bold">
          {BRAND_NAME}
        </div>
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          <a className="font-body-sm text-body-sm text-on-surface-variant hover:text-trust-navy transition-colors duration-200" href="#/">Về chúng tôi</a>
          <a className="font-body-sm text-body-sm text-on-surface-variant hover:text-trust-navy transition-colors duration-200" href="#/">Điều khoản sử dụng</a>
          <a className="font-body-sm text-body-sm text-on-surface-variant hover:text-trust-navy transition-colors duration-200" href="#/">Chính sách bảo mật</a>
          <a className="font-body-sm text-body-sm text-on-surface-variant hover:text-trust-navy transition-colors duration-200" href="#/">Liên hệ quảng cáo</a>
        </div>
